...
- Runtime principals are not table owners and cannot ALTER properties, disable append-only, TRUNCATE, DROP or replace tables. Fact writers append only. Coordination executor updates existing rows only; enrollment service alone inserts. No runtime DELETE. *(Intent — see the platform-forced deviation below for how this is expressed on Unity Catalog.)*
...
> - **Fact tables** are granted `SELECT, MODIFY` to the runtime and carry `delta.appendOnly='true'`. Because the runtime is a **non-owner** and the table is append-only, Delta rejects UPDATE/DELETE and non-ownership rejects ALTER/DROP/REPLACE — so `MODIFY` is append-only in effect. This is *not* a "broad MODIFY fallback"; it is `MODIFY` bounded by the append-only table property.
...
> - `verify_fact_permissions` / `verify_coordination_permissions` assert the effective proofs as `{SELECT, MODIFY}` accordingly, and integration tests confirm append-only rejection of UPDATE/DELETE and Serializable isolation on live tables.
