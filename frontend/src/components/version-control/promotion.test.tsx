...
it('promotion_requires_explicit_target_mapping_preflight_and_approval', async () => {
  const transport = vi.fn(async (url: RequestInfo | URL) => new Response(JSON.stringify(String(url).endsWith('/releases')
    ...
    : { operation_id: 'operation-1', status: String(url).includes('/operations/') ? 'confirmed' : 'requested', job_run_id: null, checkpoints: [], audit: [], receipt_references: [], evidence_digest: 'preflight-from-server' })))
  ...
  const valid = { ...approvalFixture, valid: true, inputs: { ...approvalFixture.inputs, target_binding: 'target', preflight_evidence_digest: 'preflight-from-server', source_version_id: 'version-1', mapping_digest: 'mapping-1', expires_at: new Date(Date.now() + 3600000).toISOString() } }
...
it('promotion_approval_binds_the_preflight_evidence_digest_returned_by_validate', async () => {
  const projection = { ...approvalFixture.inputs, target_binding: 'target', raw_source_digest: 'server-raw', rendered_target_digest: 'server-rendered', validation_policy_digest: 'server-policy' }
  const transport = vi.fn(async (url: RequestInfo | URL) => new Response(JSON.stringify(String(url).endsWith('/releases')
